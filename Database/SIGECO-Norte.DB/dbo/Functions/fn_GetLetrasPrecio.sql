create function [dbo].[fn_GetLetrasPrecio]
(
    @Numero             Decimal(18,2)
	,@p_codigo_moneda	int = 1
)
RETURNS Varchar(180)
AS
BEGIN
	DECLARE 
		@v_nombre_moneda	VARCHAR(50) = (SELECT TOP 1 UPPER(nombre) FROM dbo.moneda WHERE codigo_moneda = @p_codigo_moneda)
  ...
  BEGIN /* WHILE */
   ...
   SELECT @ImpLetra = RTRIM(@lcRetorno) + ' CON ' + SUBSTRING(@sFraccion,LEN(@sFraccion)-1,2) + '/100 ' + ISNULL(@v_nombre_moneda, 'SOLES')
  -- print @ImpLetra
   RETURN @ImpLetra
END