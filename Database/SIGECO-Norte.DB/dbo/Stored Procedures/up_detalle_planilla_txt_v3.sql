...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[up_detalle_planilla_txt_v3]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].up_detalle_planilla_txt_v3
GO
CREATE PROCEDURE [dbo].up_detalle_planilla_txt_v3
(
...
BEGIN
	...
	SELECT
		...
		rrhh.calcular_detraccion,rrhh.[checksum],rrhh.codigo_agrupador, rrhh.codigo_grupo
	FROM 
