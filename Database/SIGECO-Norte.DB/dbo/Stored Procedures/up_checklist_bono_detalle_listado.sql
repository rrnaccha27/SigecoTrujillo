...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[up_checklist_bono_detalle_listado]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].up_checklist_bono_detalle_listado
GO
CREATE PROCEDURE dbo.up_checklist_bono_detalle_listado
(
...
BEGIN
	...
	FROM
		dbo.checklist_bono_detalle ch
	INNER JOIN dbo.sigeco_reporte_bono_rrhh rh
