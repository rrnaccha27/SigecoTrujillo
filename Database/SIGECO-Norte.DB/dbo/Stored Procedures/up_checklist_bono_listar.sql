...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[up_checklist_bono_listar]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].up_checklist_bono_listar
GO

CREATE PROCEDURE dbo.up_checklist_bono_listar
AS
BEGIN
	SET NOCOUNT ON
