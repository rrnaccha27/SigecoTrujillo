CREATE PROCEDURE [dbo].[up_planilla_bono_contabilidad_planilla]
(
...
BEGIN
	...
	SELECT 
		...
		,dpb.codigo_empresa
		,dpb.codigo_moneda
		,cpb.codigo_tipo_venta
		,dpb.codigo_planilla
		,cpb.numero_contrato
		,round(((apb.dinero_ingresado * rpb.porcentaje_pago)/100)* @v_igv, 2) 
		,(select sum(x.monto_bruto) from detalle_planilla_bono x where x.codigo_planilla = @p_codigo_planilla and x.codigo_personal = dpb.codigo_personal and x.codigo_empresa = @p_codigo_empresa group by x.codigo_personal)
		,(ISNULL((select sum(x.dinero_ingresado) from articulo_planilla_bono x where x.excluido = 0 and x.codigo_planilla_bono = @p_codigo_planilla and x.codigo_personal = dpb.codigo_personal and x.codigo_empresa = @p_codigo_empresa group by x.codigo_personal), 0) * rpb.porcentaje_pago / 100)/@v_igv
		,(select monto_bruto from resumen_planilla_bono x where x.codigo_personal = dpb.codigo_personal and x.codigo_planilla = @p_codigo_planilla)
	FROM 
	...
	inner join dbo.articulo_planilla_bono apb on apb.codigo_planilla_bono = @p_codigo_planilla and apb.codigo_empresa = cpb.codigo_empresa and apb.nro_contrato = cpb.numero_contrato /*and apb.codigo_personal = cpb.codigo_personal */
	WHERE 
		...
		AND apb.excluido = 0
		--and dpb.codigo_personal in (634)
	...
	SELECT 
		...
		,ISNULL(dcont.COD_BIEN,'') as COD_BIEN
		,a.nombre as	DESCRIPCION_2
		,ISNULL(dcont.COD_CONCEPTO, '') as COD_CONCEPTO
		,'0' as	TIPO_CAMBIO
		...
		,'0' as	N_OPERACION
		,ISNULL(convert(varchar, pl.fecha_cierre, 103), '') as	FEC_CIERRE
		,'' as	DESC_TIPO_DOCUM
		,p.nro_ruc as	RUC
