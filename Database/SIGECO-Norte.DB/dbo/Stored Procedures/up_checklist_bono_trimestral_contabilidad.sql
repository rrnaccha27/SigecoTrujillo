...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[up_checklist_bono_trimestral_contabilidad]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].up_checklist_bono_trimestral_contabilidad
GO

CREATE PROCEDURE [dbo].up_checklist_bono_trimestral_contabilidad
(
...
BEGIN
	...
	SELECT TOP 1
		...
		,DIR_EMPRESA,RUC_EMPRESA,FEC_INICIO,FEC_TERMINO,DSCTO_ESTUDIO_C,DSCTO_DETRACCION,PORC_IGV,UNIDAD_NEGOCIO,codigo_canal,DIMENSION_3,DIMENSION_4,DIMENSION_5
	FROM 
		checklist_bono_trimestral_detalle chk
	INNER JOIN sigeco_reporte_bono_trimestral_contabilidad cont
