...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[up_planilla_bono_contabilidad_resumen_planilla_v3]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].up_planilla_bono_contabilidad_resumen_planilla_v3
GO

CREATE PROCEDURE [dbo].up_planilla_bono_contabilidad_resumen_planilla_v3
(
...
BEGIN
	...
	SELECT
		chk.codigo_checklist, cont.codigo_empresa, cont.NOM_EMPRESA as nombre_empresa, det.codigo_planilla, det.numero_planilla, count(cont.codigo_empresa) as bonos
	FROM 
	...
	GROUP BY
		chk.codigo_checklist, cont.codigo_empresa, cont.NOM_EMPRESA, det.codigo_planilla, det.numero_planilla
	ORDER BY 
		codigo_empresa
