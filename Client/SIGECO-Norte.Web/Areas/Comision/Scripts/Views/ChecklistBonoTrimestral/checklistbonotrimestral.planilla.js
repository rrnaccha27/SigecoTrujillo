...
function EnviarTXT(codigo_checklist) {
    var pEntidad = {
        codigo_checklist: codigo_checklist
    };
    ...
    $.ajax({
        type: 'post',
        url: ActionPlanillaUrl._EnviarTXT,
        data: JSON.stringify({ checklist: pEntidad }),
        async: true,
        cache: false,
        dataType: 'json',
        ...
        success: function (data) {
            if (data.v_resultado == 1) {
                $.messager.alert('Operación exitosa', 'Se envi&oacute; los archivos.', 'info', function () {
                });
            }
            else {
                $.messager.alert('Enviar', data.v_mensaje, 'error');
            }
        },
        error: function () {
            $.messager.alert('Error', "Error en el servidor", 'error');
