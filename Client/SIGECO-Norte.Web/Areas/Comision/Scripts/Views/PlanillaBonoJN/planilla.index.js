...
function fnNuevaPlanilla()
{
    ...
    $(this).AbrirVentanaEmergente({
        ...
        title: "Generación de Planilla Bono - Jefe Necesidad",
...
function fnModificarPlanilla() {
    ...
    $(this).AbrirVentanaEmergente({
        ...
        div: 'div_registrar_planilla',
        title: "Planilla Bono - Jefe Necesidad",
        url: ActionIndexUrl._Planilla
    });
}
...
function fnGenerarTxtPlanilla() {
    ...
    if (row.codigo_estado_planilla == 2) {
        ...
        $.messager.alert('Error', "El txt solo está permitido con planillas cerradas.", 'warning');
    }
}
...
function fnGenerarTxtPlanillaCont() {
    ...
    if (row.codigo_estado_planilla == 2) {
        $.messager.confirm('Confirmaci&oacute;n', '¿Est&aacute; seguro que desea generar el resúmen?', function (result) {
            if (result) {
                var url = ActionIndexUrl.ExportarExcel + "?id=" + row.codigo_planilla;
                //window.open(url, '_blank');
        ...
        $.messager.alert('Error', "El resúmen solo está permitido con planillas cerradas.", 'warning');
    }
}
