    ...
    jQuery.extend(app.Grupo,
        {
            ...
            Initialize: function (actionUrls) {
                ...
                $('#btnNuevo').click(function () {
                    ...
                    $('#dlgRegistrar').dialog('open').dialog('setTitle', 'Canal: ' + $('#hdNombrePadre').val() + ' - NUEVO REGISTRO');
                ...
                $('#btnModificar').click(function () {
                    ...
                    if (current.ValidarSeleccion(codigo) && current.ObtenerRegistro(codigo)) {
                        ...
                        current.InicializarGrillas();
                        $('#dlgRegistrar').dialog('open').dialog('setTitle', 'Canal: ' + $('#hdNombrePadre').val() + ' - MODIFICAR REGISTRO');
                        current.InicializarFormulario();

                        $('#dlgRegistrar #nombre').textbox('setText', Registro.nombre);
                        $('#dlgRegistrar #codigo_equivalencia').val(Registro.codigo_equivalencia);
                ...
                $('#dlgRegistrar #btnBuscarPersona').click(function () {
                    ...
                    if (!tieneSupervisor) {
                        $.messager.alert('Grupo', 'No ha configurado que este Grupo pueda tener Supervisor.', 'warning');
                        return false;
                    }

                    //if (administraGrupos) {
                    //    $.messager.alert('Grupo', 'No debe configurar Supervisor si Administra Grupos.', 'warning');
                    //    return false;
                    //}
                ...
                $('#divTransferirBotones #btnAceptar').click(function () {
                    ...
                    current.Transferir(codigo_old, 0, RegistroTransferencia.codigo_personal, codigo_canal_grupo, percibe_comision, percibe_bono);
                ...
                $('#divTransferirSupervisorBotones #btnAceptar').click(function () {
                    ...
                    if (!codigo_grupo_t && LongitudGrupo > 0) {
                        $.messager.alert('Grupo', 'Debe seleccionar un Grupo.', 'warning');
                        return false;
                    }
                    ...
                    if ( (anular && mantener && codigo_canal_grupo != '') 
                        ...
                        $.messager.alert('Grupo', 'Debe elegir solo una acci&oacute;n a tomar.', 'warning');
                        return false;
                    }

                    if (!anular && !mantener && codigo_canal_grupo == '') {
                        $.messager.alert('Grupo', 'Debe elegir una acci&oacute;n a tomar.', 'warning');
                        return false;
                    }
                    ...
                    if (!anular && mantener && codigo_canal_grupo == '') {
                        ...
                        {
                            $.messager.alert('Grupo', 'No puede elegir esta acci&oacute;n por no contar con otras supervisiones.', 'warning');
                            return false;
                        }
                        accionSupervisor = 3;
                    }
                    ...
                    if (!anular && !mantener && codigo_canal_grupo != '') {
                        if (CantidadSupervisiones != 0) {
                            $.messager.alert('Grupo', 'No puede elegir esta acci&oacute;n por contar con otras supervisiones.', 'warning');
                            return false;
                        }
            ...
            Initialize_DataGrid: function () {
                codigoPadre = $("#hdCodigoPadre").val();
                $('#DataGrid').datagrid({
                    url: project.ActionUrls.GetAllJson,
                    fitColumns: true,
                    idField: 'codigo_canal_grupo',
                    data: null,
                    queryParams: { codigoPadre: codigoPadre },
                    pagination: true,
                    singleSelect: true,
                    rownumbers: true,
                    ...
                    [[
                        ...
                        { field: 'codigo_equivalencia', title: 'C&oacute;digo', width: 60, align: 'center', halign: 'center' },
            ...
            Registrar: function () {
                ...
                var codigo_equivalencia = $.trim($("#dlgRegistrar #codigo_equivalencia").val());
                var tieneSupervisor = $('#dlgRegistrar #tieneSupervisor').switchbutton('options').checked;
                var administra_grupos = $('#dlgRegistrar #administraGrupos').switchbutton('options').checked;
                var codigoPadre = $("#hdCodigoPadre").val();
                ...
                if (!nombre) {
                    $.messager.alert('Grupo', 'Por favor ingrese un nombre.', 'warning');
                    return false;
                }
                else if (nombre.length > 50) {
                    $.messager.alert('Grupo', 'Nombre, numero maximo de caracteres 50.', 'warning');
                    return false;
                }

                if (!codigo_equivalencia) {
                    $.messager.alert('Grupo', 'Por favor ingrese un codigo unico.', 'warning');
                    return false;
                }

                if ((tieneSupervisor) && (codigoPersonal == '')) {
                    $.messager.alert('Grupo', 'Debe seleccionar un Supervisor.', 'warning');
                    return false;
                }
                ...
                //    $.messager.alert('Grupo', 'No puede configurar Supervisor y a la vez Administrar Grupos.', 'warning');
                //    return false;
                //}
                ...
                if (noPuedeGuardar) {
                    ...
                    $.messager.confirm('Confirm', '&iquest;Seguro que desea guardar este registro?', function (result) {
                        if (result) {
                            var mapData = { codigoPadre: codigoPadre, codigo_cg: codigo_canal_grupo, nombre: nombre, codigo_equivalencia: codigo_equivalencia, codigoPersonalOriginal: CodigoPersonalOriginal, codigoPersonal: codigoPersonal, administra_grupos: administra_grupos, dataConfiguracion: trama1 + trama2 + trama3 + trama4, accion: accion };
                            $.ajax({
                                    type: 'post',
                                    ...
                                    if (data.Msg) {
                                        if (data.Msg != 'Success') {
                                            $.messager.alert('Grupo', data.Msg, 'warning');
                                        }
                                        else {
                                            $('#frmRegistrar').form('clear');
                                            $('#dlgRegistrar').dialog('close');
                    ...
                    success: function (data) {
                        if (data.Msg) {
                            ...
                            Registro =
                            {
                                ...
                                codigo_equivalencia: data.codigo_equivalencia,
                                nombre: data.nombre,
            ...
            ValidarSeleccion: function (codigo) {
                if (!codigo) {
                    $.messager.alert('Grupo', 'Por favor seleccione un registro', 'warning');
                    return false;
                }
                return true;
            },
            ...
            InicializarFormulario: function () {
                ...
                personalRemovido = [];
                $('#dlgRegistrar #tabRegistrar').tabs('select', 0);
            },
