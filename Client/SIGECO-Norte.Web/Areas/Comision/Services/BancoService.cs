...
namespace SIGEES.Web.Areas.Comision.Services
{
    public class BancoService
    {
        ...
        public BancoService()
        {
            this._repository = new DataRepository<banco>(dbContext);
        }

        #region Metodos de Mantenimiento
        public IResult Create(banco instance)
        {
            if (instance == null)
            {
            ...
            try
            {
                ...
                result.IdRegistro = instance.codigo_banco.ToString();
                result.Success = true;
            }
            catch (Exception ex)
            {
        ...
        public IResult Update(banco instance)
        {
            if (instance == null)
            {
                throw new InvalidOperationException("REGISTRO NULO");
        ...
        private IQueryable<banco> GetAll(bool isReadAll = false)
        {
        ...
        private banco GetFirst(int codigo)
        {
            if (this._repository.IsExists(x => x.codigo_banco == codigo))
            {
                return this._repository.FirstOrDefault(x => x.codigo_banco == codigo);
            }
            return null;
        }
        ...
        public string GetAllJson(bool isReadAll = false)
        {
            ...
            if (allNodes.Any())
            {
                foreach (var item in allNodes)
                {
                    ...
                    {
                        {"codigo_banco", item.codigo_banco.ToString()},
                        {"nombre", item.nombre},
        ...
        public banco GetSingle(int codigo)
        {
        ...
        public string GetSingleJson(int codigo)
        {
            ...
            var jo = new JObject
            {
                {"codigo_banco", node.codigo_banco.ToString()},
                {"nombre", node.nombre},
