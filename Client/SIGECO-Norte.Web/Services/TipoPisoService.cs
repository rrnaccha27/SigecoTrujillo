...
{
    public class TipoPisoService : ITipoPisoService
    {
        private SIGEESEntities dbContext = new SIGEESEntities();

        private readonly IRepository<tipo_piso> _repository;

        public TipoPisoService()
        {
            this._repository = new DataRepository<tipo_piso>(dbContext);
        }

        /// <summary>
        /// Creates the specified instance.
        public IResult Create(tipo_piso instance)
        {
            if (instance == null)
            {
            ...
            try
            {
                ...
                result.IdRegistro = instance.codigo_tipo_piso.ToString();
                result.Success = true;
            }
            catch (Exception ex)
            {
        ...
        public IResult Update(tipo_piso instance)
        {
            if (instance == null)
            {
                throw new InvalidOperationException("REGISTRO NULO");
        ...
        public IResult Delete(tipo_piso instance)
        {
            if (null == instance)
            {
        ...
        public tipo_piso GetSingle(int id)
        {
            if (this._repository.IsExists(x => x.codigo_tipo_piso == id))
            {
                return this._repository.FirstOrDefault(x => x.codigo_tipo_piso == id);
            }
            return null;
        }
        ...
        public string GetSingleJSON(int id)
        {
            ...
            var jo = new JObject
            {
                {"codigo_tipo_piso", node.codigo_tipo_piso.ToString()},
                {"nombre_tipo_piso", node.nombre_tipo_piso},
        ...
        public IQueryable<tipo_piso> GetRegistros(bool isReadAll = false)
        {
        ...
        public string GetAllJson(bool isReadAll = false)
        {
            ...
            if (allNodes.Any())
            {
                foreach (var item in allNodes)
                {
                    ...
                    {
                        {"codigo_tipo_piso", item.codigo_tipo_piso.ToString()},
                        {"nombre_tipo_piso", item.nombre_tipo_piso},
