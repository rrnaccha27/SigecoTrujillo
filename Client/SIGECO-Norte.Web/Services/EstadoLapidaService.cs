...
{
    public class EstadoLapidaService : IEstadoLapidaService
    {
        private SIGEESEntities dbContext = new SIGEESEntities();

        private readonly IRepository<estado_lapida> _repository;

        public EstadoLapidaService()
        {
            this._repository = new DataRepository<estado_lapida>(dbContext);
        }

        public IResult Create(estado_lapida instance)
        {
            if (instance == null)
            {
            ...
            try
            {
                ...
                result.IdRegistro = instance.codigo_estado_lapida.ToString();
                result.Success = true;
            }
            catch (Exception ex)
            {
        ...
        public IResult Update(estado_lapida instance)
        {
            if (instance == null)
            {
                throw new InvalidOperationException("REGISTRO NULO");
        ...
        public IResult Delete(estado_lapida instance)
        {
            if (null == instance)
            {
        ...
        public estado_lapida GetSingle(int id)
        {
            if (this._repository.IsExists(x => x.codigo_estado_lapida == id))
            {
                return this._repository.FirstOrDefault(x => x.codigo_estado_lapida == id);
            }
            return null;
        }
        ...
        public string GetSingleJSON(int id)
        {
            ...
            var jo = new JObject
            {
                {"codigo_estado_lapida", node.codigo_estado_lapida.ToString()},
                {"nombre_estado_lapida", node.nombre_estado_lapida},
        ...
        public IQueryable<estado_lapida> GetRegistros(bool isReadAll = false)
        {
        ...
        public List<estado_lapida> Lista()
        {
        ...
        public string GetAllJson(bool isReadAll = false)
        {
            ...
            if (allNodes.Any())
            {
                foreach (var item in allNodes)
                {
                    ...
                    {
                        {"codigo_estado_lapida", item.codigo_estado_lapida.ToString()},
                        {"nombre_estado_lapida", item.nombre_estado_lapida},
        ...
        public string GetComboJson()
        {
            ...
            if (allNodes.Any())
            {
                ...
                {
                    JObject root = new JObject{
                        {"id", item.codigo_estado_lapida.ToString()}, 
                        {"text", item.nombre_estado_lapida}
                    };
                    jObjects.Add(root);
                }
            }
