...
namespace SIGEES.Web.Services
{
    public class TipoEventoService : ITipoEventoService
    {
        ...
        private readonly IRepository<tipo_evento> _repository;

        public TipoEventoService()
        {
            this._repository = new DataRepository<tipo_evento>(dbContext);
        }

        /// <summary>
        /// Creates the specified instance.
        public IResult Create(tipo_evento instance)
        {
            if (instance == null)
            {
            ...
            try
            {
                ...
                result.IdRegistro = instance.codigo_tipo_evento.ToString();
                result.Success = true;
            }
            catch (Exception ex)
            {
        ...
        public IResult Update(tipo_evento instance)
        {
            if (instance == null)
            {
                throw new InvalidOperationException("REGISTRO NULO");
        ...
        public IResult Delete(tipo_evento instance)
        {
            if (null == instance)
            {
        ...
        public tipo_evento GetSingle(int id)
        {
            if (this._repository.IsExists(x => x.codigo_tipo_evento == id))
            {
                return this._repository.FirstOrDefault(x => x.codigo_tipo_evento == id);
            }
            return null;
        }
        ...
        public string GetSingleJSON(int id)
        {
            ...
            var jo = new JObject
            {
                {"codigo_tipo_evento", node.codigo_tipo_evento.ToString()},
                {"nombre_tipo_evento", node.nombre_tipo_evento},
        ...
        public IQueryable<tipo_evento> GetRegistros(bool isReadAll = false)
        {
        ...
        public string GetAllJson(bool isReadAll = false)
        {
            ...
            if (allNodes.Any())
            {
                foreach (var item in allNodes)
                {
                    ...
                    {
                        {"codigo_tipo_evento", item.codigo_tipo_evento.ToString()},
