...
namespace SIGEES.Web.Services
{
    public class TipoBienServicioService : ITipoBienServicioService
    {
        ...
        private readonly IRepository<tipo_bien_servicio> _repository;

        public TipoBienServicioService()
        {
            this._repository = new DataRepository<tipo_bien_servicio>(dbContext);
        }

        public IResult Create(tipo_bien_servicio instance)
        {
            if (instance == null)
            {
            ...
            try
            {
                ...
                result.IdRegistro = instance.codigo_tipo_bien_servicio.ToString();
                result.Success = true;
            }
            catch (Exception ex)
            {
        ...
        public IResult Update(tipo_bien_servicio instance)
        {
            if (instance == null)
            {
                throw new InvalidOperationException("REGISTRO NULO");
        ...
        public IResult Delete(tipo_bien_servicio instance)
        {
            if (null == instance)
            {
        ...
        public tipo_bien_servicio GetSingle(int id)
        {
            if (this._repository.IsExists(x => x.codigo_tipo_bien_servicio == id))
            {
                return this._repository.FirstOrDefault(x => x.codigo_tipo_bien_servicio == id);
            }
            return null;
        }
        ...
        public string GetSingleJSON(int id)
        {
            ...
            var jo = new JObject
            {
                {"codigo_tipo_bien_servicio", node.codigo_tipo_bien_servicio.ToString()},
                {"nombre_tipo_bien_servicio", node.nombre_tipo_bien_servicio},
        ...
        public IQueryable<tipo_bien_servicio> GetRegistros(bool isReadAll = false)
        {
        ...
        public string GetAllJson(bool isReadAll = false)
        {
            ...
            if (allNodes.Any())
            {
                foreach (var item in allNodes)
                {
                    ...
                    {
                        {"codigo_tipo_bien_servicio", item.codigo_tipo_bien_servicio.ToString()},
                        {"nombre_tipo_bien_servicio", item.nombre_tipo_bien_servicio},
        ...
        public string GetComboJson()
        {
            ...
            if (allNodes.Any())
            {
                ...
                {
                    JObject root = new JObject{
                        {"id", item.codigo_tipo_bien_servicio.ToString()}, 
                        {"text", item.nombre_tipo_bien_servicio}
                    };
                    jObjects.Add(root);
                }
            }
